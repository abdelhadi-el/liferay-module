...
<div class="sidebar-header">
	...
	<c:if
		test="<%=dlViewFileVersionDisplayContext.isVersionInfoVisible()%>">
		<c:choose>
			<c:when test="<%=fileVersion.getVersion().equalsIgnoreCase("PWC") %>">
				<div title='<%=LanguageUtil.get(request, "pwc-version-tooltip")%>'
					style="float: left">
					<clay:label
						label='<%=LanguageUtil.get(request, "version")
							+ StringPool.SPACE + fileVersion.getVersion()%>'
						style="info" />
				</div>
			</c:when>
			<c:otherwise>
