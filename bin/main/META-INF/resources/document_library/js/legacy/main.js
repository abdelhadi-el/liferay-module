...
AUI.add(
	...
	A => {
		...
		var TPL_COPY_FORM =
			'<form action="{actionUrl}" method="POST"><input name="{namespace}cmd" value="copy"/>' +
			'<input name="{namespace}newFolderId" value="{newFolderId}"/>' +
			'<input name="{namespace}{parameterName}" value="{parameterValue}"/>' +
			'<input name="{namespace}redirect" value="{redirectUrl}"/>' +
			'</form>';
		...
		var DocumentLibrary = A.Component.create({
			...
			prototype: {
				...
				//custom copy functionality implemented in DMS
				_copyCurrentSelection(newFolderId) {
					var instance = this;
					...
					form.get(instance.NS + 'cmd').val('copy');
					form.get(instance.NS + 'newFolderId').val(newFolderId);
				...
				_moveSingleElement(newFolderId, parameterName, parameterValue) {
					...
					formNode.on('click', function(event){
						});
					
					submitForm(formNode, actionUrl, false);
				},
				_copySingleElement(newFolderId, parameterName, parameterValue) {
					//custom copy functionality implemented in DMS
					var instance = this;
				...
				_copyToFolder(obj) {
					...
					if (folderId) {
						if (
							...
							instance._copyCurrentSelection(folderId);
						}
					}
				},
				...
				_openModalCopy() {
					...
					this.showCopyFolderDialog(selectedItems);
				},
				...
				showCopyFolderDialog(selectedItems, parameterName, parameterValue) {
					//custom copy functionality implemented in DMS
					var instance = this;
					var namespace = instance.NS;
					...
					Liferay.Util.selectEntity(
						...
						event => {
							if (parameterName && parameterValue) {
								instance._copySingleElement(
									event.folderid,
									parameterName,
									parameterValue
								);
							} else {
								instance._copyCurrentSelection(event.folderid);
							}
						}
					);
				}
